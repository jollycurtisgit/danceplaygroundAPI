...
async function main(){
  await MongoUtil.connect(MONGO_URI, "danceplaygroundAPI");
   ...
   // 1. working - Display all added classes  
  ...
  //3. Wonderful! It's working! - Add a class
 ...
 //6. working - Display all 30 mins classes  
 ...
 //7. working - Display all 1hr classes  
...
//8. working - Display all 2hr classes  
...
//
 ...
 app.get("/durationClassesSearch", async function (req, res) {
  ...
  res.send(classRecords);
    ...
    // Trial- Post a user
   app.post("/SignUpTry", async function (req, res) {
    const db = MongoUtil.getDB();
    const postAmember = {
     fName: req.body.fName,
     lName: req.body.lName,
    ...
    let emailRecords = await db.collection('SignUp').find(
      {"email":req.body.email}
      ).toArray();
